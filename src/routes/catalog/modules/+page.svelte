<!-- src/routes/catalog/modules/+page.svelte -->
<script lang="ts">
  ...
  const startCursor: string = '1000' /* Начало диапазона DevID для данной категории */
  const endCursor: string = 'AFFF' /* Конец диапазона DevID для данной категории */
...
<!-- Разметка компонента -->
<h2>{t('catalog.modules.title', currentLang)}</h2>
