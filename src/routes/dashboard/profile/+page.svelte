...
<script lang="ts">
  ...
  import { UserStore, ThemeStore, UserClear, UserClearTemp, DeviceListClear, NewsListClear, LoaderStore, addMessage } from '../../../stores'
    ...
    <div class="flex flex-wrap items-start overflow-y-auto">
      ...
      >
        ...
        <Input
          ...
          label={t('dashboard.profile.department', currentLang)}
          className="w-full"
          bind:value={$UserStore.Department}
        />
        <TextArea
          id="aboutMe"
...
{#if currentLang && UserData}
  <ConfirmDelete show={showModalDelete} item={UserData.UserID} {currentLang} onConfirm={confirmDelete} onCancel={() => (showModalDelete = false)} />
{/if}
